#![no_std]
#![feature(alloc_error_handler, panic_info_message, llvm_asm)]
...
extern "C" fn kinit() -> usize {
    ...
    let root_ptr = mem::get_page_table();
    let root_u = root_ptr as usize;
    let mut root = unsafe { root_ptr.as_mut().unwrap() };
    let kheap_head = mem::get_head() as usize;
    let total_pages = mem::get_num_allocations();
    println!();
    println!();
    unsafe {
